...
class XLCHLinearSlidersElement extends HTMLElement {
    ...
    if (this.#gamutHints === "none") {
      ...
      // Determine ranges
      {
        ...
        for (let column = 0; column <= width; column += step) {
          let h = (column / width);
          let [x, y, z] = convertColor({space: this.#space, coords: [l, c, h*360]}, "xyz-d65").coords;
  ...
  async #updateChromaSliderGamutPath() {
    if (this.#gamutHints === "none") {
      ...
      // Determine ranges
      {
        ...
        for (let column = 0; column <= width; column += step) {
          ...
          let [x, y, z] = convertColor({space: this.#space, coords: [l, c * maxChroma, h]}, "xyz-d65").coords;
          let inGamut = await this.#ownerColorPicker.isColorInGamut(x, y, z, this.#gamutHints);
    ...
    if (this.#gamutHints === "none") {
      ...
      // Determine ranges
      {
        ...
        for (let column = 0; column <= width; column += step) {
          ...
          if (this.#space === "lch") {
            let [x, y, z] = convertColor({space: this.#space, coords: [l*100, c, h]}, "xyz-d65").coords;
            inGamut = await this.#ownerColorPicker.isColorInGamut(x, y, z, this.#gamutHints);
          }
          else if (this.#space === "oklch") {
            let [x, y, z] = convertColor({space: this.#space, coords: [l, c, h]}, "xyz-d65").coords;
            inGamut = await this.#ownerColorPicker.isColorInGamut(x, y, z, this.#gamutHints);
          }
